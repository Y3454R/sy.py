...
Let's break it down. We created an array `self.enc_sizes` that holds the sizes of our encoder. Then we create an array `conv_blocks` by iterating the sizes. Since we have to give both a in size and an outsize for each layer we `zip`ped the size array with itself by shifting it by one. 
